...
module.exports = Document => {
  createRemoteMethod({
    ...
    name: 'uploadIdentity',
    accepts: [],
    description: 'upload identity for an user',
    httpOptions: {
      errorStatus: 400,
      path: '/uploadIdentity',
      status: 200,
  ...
  Document.uploadIdentity = async (request, response, callback) => {
    try {
      ...
      const existingDocument = documents.find(d => d.type === 'identity');
      if (existingDocument) {
      ...
      const existingProofDocument = await Document.findOne({
        where: {
          userId: targetUser.id,
          type: 'proof'
        }
      });
      if (existingProofDocument && targetUser.verificationStatus === 'unverified') {
